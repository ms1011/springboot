...
public class FileUploadController {
    ...
    public String singleFileUpload(@RequestParam MultipartFile singleFile,
        ...
        Resource resource = resourceLoader.getResource("classpath:static/uploadFile/img/single");
//        System.out.println("빌드 된 single 디렉토리의 절대경로 : " + resource.getFile().getAbsolutePath());
        String filePath = resource.getFile().getAbsolutePath();

        /* 설명. 사용자가 넘긴 파일을 확인하고 rename해 보자 (날짜 방식이 아닌 UUID 클래스를 활용해서 rename) */
        ...
        /* 설명. 우리가 지정한 경로로 파일을 저장하고, FlashAttribute에 값 전달 */
        try {
            ...
            rttr.addFlashAttribute("img", "uploadFile/img/single/" + savedName);
...
//        System.out.println("multiFileDescription = " + multiFileDescription);
        ...
        String filePath = resourceLoader.getResource("classpath:static/uploadFile/img/multi")
                                        .getFile()
                                        .getAbsolutePath();
        ...
        try {
            for (int i = 0; i < multiFiles.size(); i++) {
                ...
                saveFiles.add("uploadFile/img/multi/" + savedName);
            }
            ...
            /* 설명. List<Map<String, Sting>> files로 쌓인 업로드 된 파일들을 찾아 하나씩 다시 지운다 */
